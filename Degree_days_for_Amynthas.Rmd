...
dailymeans.df$GDD5 <- dailymeans.df$meantemp - 5  #create new column by subtracting 5 from meantemp.
dailymeans.df <- dailymeans.df %>% filter(GDD5 > 0)
...
meta.df <- str_split_fixed(dailymeans.df$name, "_", n = 5)
#head(meta.df)

# Name and attach the columns from meta.df
dailymeans.df <- dailymeans.df %>% add_column(site = meta.df[,1], rep = meta.df[,2], position = meta.df[,3], buttonID = meta.df[,4], season = meta.df[,5],
                         .before = "date")
head(dailymeans.df)
